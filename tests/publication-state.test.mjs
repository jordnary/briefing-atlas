...
const stateFixture = () => ({
  version: 1, records: {}, pending: [], sourceCommit: binding.sourceCommit,
  checkpointBinding: { ...binding }, publication: { stage: 'archived', binding: { ...binding } },
});
...
test('every publication operation requires the exact public commit, archive version and private source commit', () => {
  for (const field of ['commit', 'archiveVersion', 'sourceCommit']) {
    ...
    const stale = { ...binding, [field]: 'd'.repeat(field === 'archiveVersion' ? 64 : 40) };
    assert.throws(() => publicationPlan(state, stale), /PUBLICATION_CHECKPOINT_BINDING_MISMATCH/);
    assert.throws(() => transitionPublication(state, stale, { type: 'built' }), /PUBLICATION_CHECKPOINT_BINDING_MISMATCH/);
  }
  const state = stateFixture();
  delete state.checkpointBinding;
  assert.throws(() => assertPublicationBinding(state, binding), /PUBLICATION_CHECKPOINT_BINDING_MISMATCH/);
  assert.throws(() => assertPublicationBinding(state, {}), /PUBLICATION_BINDING_REQUIRED/);
});
...
test('saved deployment intent blocks retries even for the same run after uncertain external failure', () => {
  ...
  apply(state, { type: 'built' });
  apply(state, { type: 'begin-deploy', attempt: 'run-1' });
  apply(state, { type: 'failed', stage: 'deploy', error: new Error('DEPLOYMENT_FAILED') });
  assert.equal(state.publication.deploymentIntent.attempt, 'run-1');
  assert.throws(() => publicationPlan(state, binding), /PUBLICATION_DEPLOYMENT_RECOVERY_REQUIRED/);
  for (const attempt of ['run-1', 'run-2'])
    assert.throws(() => transitionPublication(state, binding, { type: 'begin-deploy', attempt }), /PUBLICATION_DEPLOYMENT_RECOVERY_REQUIRED/);
  assert.throws(() => transitionPublication(state, binding, { type: 'deployed', receipt, attempt: 'run-2' }), /PUBLICATION_ATTEMPT_MISMATCH/);
  apply(state, { type: 'deployed', receipt, attempt: 'run-1' });
...
test('identical receipts are idempotent and delayed failures cannot regress verified publication', () => {
  ...
  for (const stage of ['build', 'deploy', 'verify'])
    assert.throws(() => transitionPublication(state, binding, { type: 'failed', stage, error: new Error('LATE_FAILURE') }), /PUBLICATION_STALE_FAILURE/);
  assert.throws(() => transitionPublication(state, binding, { type: 'deployed', receipt: { ...receipt, id: 'deployment-2' } }), /PUBLICATION_RECEIPT_CONFLICT/);
...
test('health drift preserves verified receipts and schedules verification without another deployment', () => {
  ...
  apply(state, { type: 'health', code: 'ONLINE_VERSION_DRIFT', at: '2026-09-12T00:00:00Z' });
  ...
  assert.deepEqual(publicationPlan(state, binding), {
    should_publish: true, should_build: false, should_deploy: false, should_verify: true,
    version: binding.archiveVersion, page_url: receipt.url, deployment_id: receipt.id,
  });
  apply(state, { type: 'verified' });
...
test('retry stages and malformed publication receipts are rejected before an external action', () => {
  const state = stateFixture();
  assert.throws(() => publicationPlan(state, binding, { retryStage: 'verify' }), /PUBLICATION_RETRY_STAGE_MISMATCH/);
  deploy(state);
  assert.throws(() => publicationPlan(state, binding, { retryStage: 'build' }), /PUBLICATION_RETRY_STAGE_MISMATCH/);
  for (const change of [
    ...
    assert.throws(() => publicationPlan(corrupt, binding), /INVALID_PUBLICATION_STATE/);
  }
});
...
test('empty, malformed and structurally corrupt local state fail explicitly without checkpoint fallback', async (t) => {
  ...
  const backup = await readFile(path.join(privateDir(root), 'checkpoint.json'), 'utf8');
  for (const content of ['', '{"broken":', 'null', JSON.stringify({ ...stateFixture(), publication: {} })]) {
    await writeFile(path.join(privateDir(root), 'state.json'), content);
    await assert.rejects(readState(root), /INVALID_PRIVATE_STATE/);
    assert.equal(await readFile(path.join(privateDir(root), 'checkpoint.json'), 'utf8'), backup);
  }
  const different = stateFixture();
  different.publication.failedStage = 'build';
  await writeFile(path.join(privateDir(root), 'state.json'), JSON.stringify(different));
...
test('corrupt recovery journals are rejected before writing their targets', async (t) => {
  const root = await workspace(t);
  await saveState(root, stateFixture());
  const before = await readFile(path.join(privateDir(root), 'state.json'), 'utf8');
  for (const batch of ['{', JSON.stringify({ version: 1, entries: [{ date: '../outside' }], state: stateFixture() })]) {
    await writeFile(path.join(privateDir(root), 'batch.json'), batch);
    await assert.rejects(recoverBatch(root), /CORRUPT_BATCH/);
    assert.equal(await readFile(path.join(privateDir(root), 'state.json'), 'utf8'), before);
  }
});
...
test('competing local publication writers cannot overwrite the active writer', async (t) => {
  ...
  await withArchiveLock(root, async () => {
    await assert.rejects(withArchiveLock(root, async () => assert.fail('second writer ran')), /ARCHIVE_LOCKED/);

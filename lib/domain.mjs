      ...
      terms.every((term) =>
        normalize(
          [
            story.title,
            story.summary,
            readingText(story.body),
            readingText(story.briefingIntro || ''),
            readingText(story.briefingOutro || ''),
            ...story.tags,
            ...story.entities,
          ].join(' '),
        ).includes(term),
...
export function validateReadingState(input) {
  ...
  if (
    ...
    !['standard', 'wide'].includes(reader.width) ||
  ...
  return {
    ...
    view: input.view,
    reader: { font: reader.font, width: reader.width, spacing: reader.spacing },
    lastRead,
  };
}

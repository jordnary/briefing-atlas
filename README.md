...
当前内容为原任务已发送的 **11 期、55 条**，日期为 2026-08-28 至 2026-09-08。2026-09-05 本次未读到，未生成补位内容。收录成功不等于事实核验。
...
索引加载后校验稳定 ID、日期、必要字段类型、来源 URL 和核验状态；格式异常时提供重试。搜索摘录来自正文或元数据文字，排除图库说明和 Markdown 链接目标。`search_briefing_archive` WebMCP 工具复用同一解析和评分逻辑，返回 `{ total, offset, hasMore, results }`；默认每页 20 条，最多 100 条，支持排序和自定义权重。

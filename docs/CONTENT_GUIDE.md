...
- **相关性第一**：优先直接对应新闻中的人物、公司、产品、项目、论文、游戏或具体事件。
- **信息量优先**：优先产品界面、演示截图、架构图、数据图、论文图表和游戏实机，纯 Logo 或泛化装饰图只在没有更好素材时使用。
...
- **多图必须互补**：同一条新闻的多张图应分别说明不同信息，例如“产品 + 界面 + 发布现场”，避免重复堆叠相似宣传图。
- **可核验且适合展示**：图片应清晰、主体明确、辨识度高，并保留图片来源链接；找不到强相关、可信且不误导的素材时，宁可只保留少量图片或留空。

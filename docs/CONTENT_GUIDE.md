...
# 原稿修订先生成私有差异，审阅后才接受
# workflow dispatch: sync-source.yml action=approve with review_id and source_commit
...
原图可在正文使用普通 Markdown 图片，或在 `:::gallery` 块中按顺序排列图片与来源链接。远程原图引用不代表已核验转载许可；需要长期稳定展示时，先确认许可，再下载到 `public/images/`。原有 `image` 字段仍支持 `path`、`alt`、`caption`、`source`。
...
同日期不同正文不能覆盖。原消息编辑先生成带唯一 review ID 的完整前后差异；人工批准时必须通过 workflow dispatch 提交该 review ID、source commit，系统重新校验导出与检查点后才接受。标题或正文完全匹配时复用编号；两者同时变化且无法可靠对应时，回执中提供经过审阅的 `storyMapping`，键为原稿新顺序编号，值为已有新闻编号。增删条目也需人工设计保持旧链接的迁移，默认暂停。若确实新增条目，可在 `storyMapping` 中为新增位置指定 `briefing-YYYY-MM-DD-NN` 形式、且尚未使用的本期 ID；所有旧条目仍必须各自映射一次，否则继续暂停并返回 `STORY_MAPPING_REQUIRED`。
...
每条新闻都主动搜索合适的视觉素材，至少争取 1 张；通常使用 1–3 张，重点新闻在确有互补信息时可扩展到 2–5 张。数量服从质量，不能为了达到数量加入弱相关、重复或仅作装饰的图片。

选图按以下顺序判断：

- **数量门槛**：每条新闻默认展示 3–5 张互补图片；重点新闻在确有不同信息价值时可扩展到 5–7 张。正式交付每条至少 2 张；确实找不到第二张合格图片时，必须在该条的“配图归档说明”中说明原因，找不到任何合格图片时应暂缓该条。
- **相关性第一**：至少一张图应直接对应新闻中的人物、公司、产品、项目、论文或具体事件；其余图片应补充界面、架构、数据、发布现场、产品细节或可信背景。
- **信息量与质量优先**：优先高分辨率、主体清晰、构图有信息量的官方图、产品界面、演示截图、架构图、数据图、论文图表和游戏实机。纯 Logo、股票图、通用办公场景和泛化装饰图不能用来补足数量。
- **一手来源优先**：优先官方博客、官网、论文原文、GitHub 和发布会素材，再考虑可信媒体图片或 Wikimedia Commons 等可核验来源。
- **主题相关但不误导**：没有事件原图时，可以使用明确相关的产品、人物或项目图片；标题、替代文本和来源说明必须标明它是主题相关图，不让读者误以为是本次现场或最新发布内容。
- **多图必须互补**：同一条新闻的多张图应分别说明不同信息，例如“产品 + 界面 + 发布现场”，避免重复堆叠相似宣传图、同一图片或同一图库素材。
- **可核验且适合展示**：图片应清晰、主体明确、辨识度高，并保留图片来源链接。数量下限不是使用通用示意图凑数的理由；如果只能达到数量而不能达到相关性和质量要求，应减少条目或暂缓交付。

补入的图片应在图库中保留准确的替代文本、简短说明和来源链接。图片来源页是归因入口，不等同于已经取得转载许可；需要长期托管时，先确认许可再提交本地副本。

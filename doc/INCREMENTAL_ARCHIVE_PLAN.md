...
| 项目 | 已有证据 | 对新方案的影响 |
| --- | --- | --- |
| 原文读取 | 本会话可读到 11 期历史正文，范围为 2026-08-28 至 2026-09-08；旧核对记录统计为 55 条 | 具备一期试迁移输入；不代表已建立长期自动读取通道 |
| 缺档 | 当前返回记录没有 2026-09-05 | 列为“本次未读到”，继续补查，不能据此判定任务未运行或生成填充稿 |
...
- 区分原稿发送时间、网站归档时间、原稿修订时间和发布完成时间。原稿时间未知允许为空，不将计划运行的 08:00 当作实际发送时间。
...
| `sourcePublishedAt` | 可空；记录能够确认的原稿发送时间 |
| `archivedAt`、`revision`、`corrections` | 归档文件；区分保存时间和修订，显示必要更正说明 |
...
| P2：历史迁移与示例退出 | 枚举可读历史、记录缺档、批量导入；移走正式示例并调整测试夹具 | 日期清单和条目数可对账；2026-09-05 未查明前保持缺档；正式索引无示例 |
...
- 未恢复的引用、原图和未知时间均有真实状态；没有新写的新闻、伪造的来源或填充日期。
